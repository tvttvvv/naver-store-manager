...
def get_naver_shopping_rank(keyword, store_name):
    ...
    def fetch_html_stealth(url):
        try:
            ...
            if r.status_code == 200 and len(r.text) > 500:
                ...
                print("[ScraperAPI 오류] 상태코드: 403 - 무료 크레딧이 만료되었거나 한도 초과입니다.")
            else:
    ...
    try:
        ...
        if not unique_mids:
            ...
            return {'rank': 'ScraperAPI 통신 실패', 'title': '', 'link': '', 'price': ''}
                ...
                try:
                    ...
                    if kw_update and updates:
                        ...
                        if update_mode in ['all', 'rank'] and 'store_rank' in updates:
                            if kw_update.store_rank != updates['store_rank'] and kw_update.store_rank not in ['-', '에러', '실패', '검색 차단됨', '모든 통신망 차단됨', '포털 검색 차단됨', '포털 보안망 차단됨', '초강력 보안망 완전 차단됨', '시스템 점검/네트워크 오류', '카탈로그 차단됨', '검색 오류', '검색결과 없음', '검색결과 없음(구조변경됨)', '매칭중', 'ScraperAPI 통신 실패'] and "밖" not in kw_update.store_rank:
...
def get_task_status():
    ...
    return jsonify(task)                    'premium': 'true'         # 강력한 차단 우회망
                }
                r = requests.get('http://api.scraperapi.com/', params=payload, timeout=45)
                r.encoding = 'utf-8'
